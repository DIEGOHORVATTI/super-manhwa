const mangayomiSources = [
  {
    ...
    // VENDOR PATCH: api.comick.fun is dead (connection refused). The current API
    // host is comick.dev (api.comick.io 301-redirects there). It sits behind
    // Cloudflare, so requests must route through FlareSolverr (connector is
    // hasCloudflare: true). See EXTENSIONS_SYNC.md.
    apiUrl: "https://comick.dev",
    iconUrl:
      "https://raw.githubusercontent.com/m2k3a/mangayomi-extensions/main/javascript/icon/all.comick.png",
    typeSource: "single",
    itemType: 0,
    version: "0.1.0",
...
class DefaultExtension extends MProvider {
  ...
  async getDetail(url) {
    ...
    const chapUrlReq = `${this.source.apiUrl}${url.replaceAll("#", "")}chapters?${lang}&tachiyomi=true&page=1`;
    const total = JSON.parse((await this.client.get(chapUrlReq, this.getHeaders())).body).total;
    const newChapUrlReq = `${this.source.apiUrl}${url.replaceAll("#", "")}chapters?limit=${parseInt(total, 10)}${lang}&tachiyomi=true&page=1`;
    ...
    const chapters = JSON.parse(newRes.body).chapters.map((chapter) => {
      ...
      if (chapter.chap !== "null" && chapter.vol !== "null") {
        title = this.beautifyChapterName(chapter.vol, chapter.chap, chapter.title);
      } else {
        title = chapter.title;
      }

      if (chapter.group_name && chapter.group_name !== "null") {
        scanlator = chapter.group_name.toString().replace(/]/g, "").replace(/\[/g, "");
      }
