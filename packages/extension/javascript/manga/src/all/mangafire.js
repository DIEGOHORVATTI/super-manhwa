...
class DefaultExtension extends MProvider {
  ...
  async getDetail(url) {
    ...
    // VENDOR PATCH: our runtime has no preferences UI, so getPreference returns
    // undefined → viewType becomes undefined → the vrf key + ajax JSON break
    // ("cannot read property '0' of undefined"). Default to "chapter" so detail
    // + chapter list resolve. See EXTENSIONS_SYNC.md for the re-sync caveat.
    ...
    const chapterUrl =
      this.source.baseUrl + `/ajax/read/${id}/${viewType}/${this.source.lang}?vrf=${vrf}`;
    ...
    if (viewType == "chapter") {
      ...
      const chapRes = await new Client().get(
        this.source.baseUrl + `/ajax/manga/${id}/${viewType}/${this.source.lang}?vrf=${vrf}`,
      );
  ...
  async getPageList(url) {
    const fetch = async (chapid, viewType) => {
      ...
      const reqUrl = this.source.baseUrl + `/ajax/read/${viewType}/${chapid}?vrf=${vrf}`;
  ...
  fromBytes = (bytes) => bytes.map((b) => String.fromCharCode(b & 0xff)).join("");
