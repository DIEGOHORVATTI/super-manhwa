...
class DefaultExtension extends MProvider {
  ...
  _cacheAuthTokensFromResponse(response) {
    const requestCookieHeader = this._getHeaderValue(response?.request?.headers, "cookie");
    const responseSetCookieHeader = this._getHeaderValue(response?.headers, "set-cookie");
  ...
  async _refreshAccessToken() {
    ...
    try {
      ...
      if (data?.access_token) this._accessTokenCache = String(data.access_token);
      if (data?.refresh_token) this._refreshTokenCache = String(data.refresh_token);
  ...
  _chapterRefFromUrl(url) {
    ...
    if (parts.length >= 3 && (parts[0] === "comics" || parts[0] === "series") && parts[1]) {
      return {
  ...
  async getLatestUpdates(page) {
    ...
    return this._parseMangaList(this._parseJsonBody(res.body, "getLatestUpdates"));
  }
  ...
  async getDetail(url) {
    ...
    const seriesRes = await this._apiGet(`${this.apiBase}/api/series/${slug}`, true);
    ...
    while (true) {
      ...
      const pageJson = this._parseJsonBody(chapRes.body, `getDetail-chapters-offset-${offset}`);
    ...
    const chapters = allChaps.map((ch) => {
      const chapterLabel = ch.number != null ? `Chapter ${ch.number}` : "Chapter";
  ...
  async getPageList(url) {
      ...
      return sourcePages
        .map((p, i) =>
          typeof p === "string"
            ? { url: p, order: i }
            : {
                url: p.url || p.image_url || p.page_url || p.image || p.src || "",
    ...
    const fetchChapter = async (context) => {
      ...
      const isLocked = chapter?.is_locked ?? json.data?.is_locked ?? json.is_locked ?? false;
    ...
    if (pageList.length === 0 && chapterData.isLocked && this._accessTokenCache) {
    ...
    if (pageList.length === 0 && chapterData.isLocked && this._refreshTokenCache) {
    ...
    if (!hasSessionAuth) {
      throw new Error("Chapter appears locked. Login via source WebView, then retry.");
    }
